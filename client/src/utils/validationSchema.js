...
export const validationSchemaRegisterForm = Yup.object().shape({
  license: Yup.boolean().oneOf([true], 'You must agree to the terms and conditions'),
  ...
  password: Yup.string()
    ...
    .required('Field is required'),
  email: Yup.string().matches(emailRegex, 'Invalid email address').required('Field is required'),
  lastName: Yup.string()
    .matches(
      nameRegex,
      'The name must begin with a capital letter, consist only of latin letters and must be between 2 and 15 characters long'
    )
    .required('Field is required'),
...
export const validationSchemaUserDataForm = Yup.object().shape({
  ...
  phoneNumber: Yup.string().matches(/^\d{12}$/, 'Phone number must be 12 digits'),
...
export const validationSchemaNPDeliveryAddress = Yup.object().shape({
  ...
  street: Yup.string()
    .matches(
      streetNameRegex,
      'Street name must start with a capital latin letter, can include one space or hyphen and must be up to two words'
    )
  ...
  houseNumber: Yup.string()
    ...
    .matches(
      /^[0-9A-Za-z/]+S?$/,
      'House number must consist only numbers, latin letters and dont include spaces or hyphens'
    )
    .matches(
      /^[0-9A-Za-z]+(\/[0-9A-Za-z]+)?$/,
      'House number format must be number of number/number'
    )
    .required('House number is required'),
...
export const validationSchemaNPDeliveryInternational = Yup.object().shape({
  intDeliveryAddress: Yup.string()
    .matches(
      /^[a-zA-Z0-9.,/ ]*$/,
      'Address must consist only numbers and latin letters and include postal code, city, street and house number'
    )
    .min(
      40,
      'Address must be at least 40 characters long and include postal code, city, street and house number'
    )
    .max(
      100,
      'Address must be at most 100 characters long and include postal code, city, street and house number'
    )
    .matches(
      /^(?!\s*$).+/,
      'Address must not be empty and include postal code, city, street and house number'
    )
