...
const LoginFormBlock = ({ type }) => {
  ...
  const redirectPath =
    location.pathname === '/cart/checkoutUserInfo' ? '/cart/checkoutUserInfo' : '/shop/';
    ...
    onSuccess: (tokenResponse) => {
      ...
      googleService.get(accessToken).then((userInfo) => {
        dispatch(signInWithGoogle(userInfo)).then((response) => {
          response.type === 'user/signInWithGoogle/fulfilled' &&
  ...
  return (
    <>
      <section className={styles.loginFormBlock} data-testid="LoginFormBlock" type={type}>
        ...
        <form className={styles.loginFormBlock__form} onSubmit={formik.handleSubmit}>
          <TextField
            label="Email"
            name="email"
          ...
          >
            {isLoading && isRegularLogin ? <LoaderIconSmall /> : <span>Sign In</span>}
          </button>
          <Link
            to="recoveryPassword"
