...
import UnknownUserContactFormBlock from '../../../../components/form/formBlocks/UnknownUserContactFormBlock/UnknownUserContactFormBlock';
...
const UserInfoBlock = () => {
  ...
  return (
    <div className={styles.checkOutUserInfoBlock}>
      ...
      >
        {userCurrentDetailsList.map((detail, index) => (
          <div key={index}>
            <section key={detail.id} className={styles.checkOutUserInfoBlock__radioWrapper}>
              <input
                id={detail.id}
                type="radio"
                name="customRadio"
      ...
      <form
        ...
        className={styles.checkOutUserInfoBlock__checkOutUserInfoBlock}
        data-testid="UserInfoBlock"
      >
        {userCurrentDetails === '1' && userCurrentDetailsList[0].value}
      </form>
      <div className={styles.checkOutUserInfoBlock__navigationButtonsSection}>
        <NavLink aria-label="go back" title="go back" to={'/cart'}>
          <div className={styles.checkOutUserInfoBlock__buttonLeft}>
            <LeftArrowIcon />
            <span>go back</span>
          </div>
        </NavLink>
