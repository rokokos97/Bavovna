...
const ResetPasswordForm = () => {
  ...
  return (
    <article className={styles.resetPasswordForm} data-testid="ResetPasswordForm">
      <section className={styles.resetPasswordForm__titleBlock}>
        <p>Reset password</p>
      ...
      {response ? (
        <section
          className={
            response.code !== 200
              ? styles.resetPasswordForm__errorMessagesBlock
              : styles.resetPasswordForm__successMessagesBlock
          }
        >
          <p>{transformErrorMessage[response.message]}</p>
        </section>
      ) : null}
      <form className={styles.resetPasswordForm__form} onSubmit={formik.handleSubmit}>
        <TextField
          type="password"
          label="Password"
          name="password"
        ...
        >
          {isLoaderRun ? <LoaderIconSmall /> : <span>Reset</span>}
        </button>
      </form>
    </article>
  );
